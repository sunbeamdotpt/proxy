...
### Features
- feat(telemetry): export spans to an OTLP collector over OTLP/HTTP
- feat(config): add x_forwarded_for option (disabled by default)

### Bug Fixes
- fix(telemetry): append /v1/traces to otlp_endpoint when missing
- fix(telemetry): configure JSON log layer with JsonFields for span events

### Testing
- test(otel): add collector integration test via sunbeam-test
- test(tls): generate not-yet-valid certificate at test time

### Build & CI
- build(release): detect Docker socket for integration tests

### Documentation
- docs(changelog): add unreleased section
- docs(changelog): backfill v0.2.0 and v0.2.1 release notes
